...
uint16_t sys_fork(){
	...
	if((uint64_t)cproc){
		...
		cproc->mm=(struct mm_struct *)((char *)(&cproc + 1));
		cproc->mm->cnt=0;
	...
	//		process->kstack[j]=pproc->process->kstack[j];
		...
		memset(cproc->kstack,'\0',4096);
		//proc->process->kstack[491] = (uint64_t)(&irq0+34);
        cproc->kernel_rsp = (uint64_t *)&cproc->kstack[490];
		for(i=0;i<512;i++){
			cproc->kstack[i]=pproc->kstack[i];
		}

		//process->kstack[511] = 0x23 ;                              //SS    
          cproc->kstack[510] = (uint64_t)(&cproc->stack[511]);      //  ESP
		cproc->kstack[509] = 0x200286;                           // EFLAGS - 0x200286
		...
		cproc->kstack[506] =0;  // assigning rax of child to zero;
		cproc->kernel_rsp = (uint64_t *)(&cproc->kstack[490]);
		__asm__ __volatile__ ("movq %0, %%cr3":: "a"(cproc->cr3_address));
