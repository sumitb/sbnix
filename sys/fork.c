...
uint16_t sys_fork(){
	struct task_struct *pproc = runningTask;
	...
	//		process->kstack[j]=pproc->process->kstack[j];
		...
		//copy stack
		//uint64_t *temp = cproc->stack;
		//cproc->stack = pproc->stack;
       ...
       //   cproc->kstack[62] = (uint64_t)(&cproc->stack[63]);      //  ESP
	    cproc->kstack[62] = (uint64_t)(&cproc->stack[511]);     //  ESP
